...
4.添加了点击画布选择图元, 试图使用QGraphicsScene的itemAt方法,发现两个item重叠多的时候很难选,于是改成记录图元的所有点,与鼠标点击的坐标逐一比对.此外,清除对图元的选择点击右边clear selection即可.\\
...
9.限制鼠标点击事件的点在[0,g\_width], [0,g\_height]之间, 不然经常往外拖就会崩溃.同时,如果图元被操作到完全处于边界外,比如平移,接下来对它的任何编辑操作都不起作用,也就是说无法再移回来了.\\
